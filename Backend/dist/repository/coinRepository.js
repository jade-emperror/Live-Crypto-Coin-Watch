...
export async function deactivateCoin(symbol) {
    await CoinModel.findOneAndUpdate({ symbol: symbol }, { view: false });
}
export async function toggleOrInsertCoin(symbol) {
    await CoinModel.findOneAndUpdate({ symbol: symbol }, { view: true }, {
        upsert: true,
        runValidators: true
    });
}
